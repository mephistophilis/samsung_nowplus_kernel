...
int nowplus_enable_touch_pins(int enable) {
	...
	if (enable) {
		omap34xx_pad_set_config_lcd(PAD_OFFS_I2C3_SCL,
				OMAP34XX_MUX_MODE0 | OMAP34XX_PIN_INPUT_PULLUP
						| OMAP34XX_PIN_OFF_OUTPUT_LOW);
		omap34xx_pad_set_config_lcd(PAD_OFFS_I2C3_SDA,
				OMAP34XX_MUX_MODE0 | OMAP34XX_PIN_INPUT_PULLUP
						| OMAP34XX_PIN_OFF_OUTPUT_LOW);
	} else {
		omap34xx_pad_set_config_lcd(PAD_OFFS_I2C3_SCL,
...
int nowplus_enable_uart_pins(int enable) {
	...
	if (enable) {
		omap34xx_pad_set_config_lcd(PAD_OFFS_UART3_RX,
				OMAP34XX_MUX_MODE0 | OMAP34XX_PIN_INPUT_PULLUP
						| OMAP34XX_PIN_OFF_INPUT);
		omap34xx_pad_set_config_lcd(PAD_OFFS_UART3_TX,
...
static int nowplus_twl4030_keymap[] = { 
	KEY(0, 0, KEY_FRONT), KEY(1, 0,	KEY_SEARCH), KEY(2, 0, KEY_CAMERA_FOCUS), 
	KEY(0, 1, KEY_PHONE), KEY(2, 1, KEY_CAMERA), KEY(0, 2, KEY_EXIT), 
	KEY(1, 2, KEY_VOLUMEUP), KEY(2, 2, KEY_VOLUMEDOWN), 
};
...
static struct omap_dss_device *nowplus_dss_devices[] = { &nowplus_lcd_device, };
...
/*	0	C*/
30800, 29500, 28300, 27100, 26000, 24900, 23900, 22900, 22000, 21100, 20300,
		19400, 18700, 17900, 17200, 16500, 15900, 15300, 14700, 14100, 13600,
		13100, 12600, 12100, 11600, 11200, 10800, 10400, 10000, 9630, 9280,
		8950, 8620, 8310, 8020, 7730, 7460, 7200, 6950, 6710, 6470, 6250, 6040,
		5830, 5640, 5450, 5260, 5090, 4920, 4760, 4600, 4450, 4310, 4170, 4040,
		3910, 3790, 3670, 3550 };
...
static void nowplus_init_camera(void) {
	if (gpio_request(OMAP_GPIO_CAMERA_LEVEL_CTRL, "OMAP_GPIO_CAMERA_LEVEL_CTRL")
			!= 0) {
...
static void *dev_attr[] = { &dev_attr_debug_level, &dev_attr_block_popup, };
